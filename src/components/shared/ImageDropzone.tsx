...
function ImageDropzone({ onFilesAdded, onFileDelete, photos }: InputProps) {
  ...
  return (
    <>
      <div className='grid-cols[repeat(1,1fr) grid gap-2 sm:grid-cols-[repeat(2,1fr)] lg:grid-cols-[repeat(3,1fr)]'>
        <div
          {...getRootProps()}
          className='cursor-pointer border-2 border-dashed border-slate-200 p-2 text-center'
        >
          <input {...getInputProps()} />
          <div className='text-gray=400 flex flex-col items-center justify-center text-xs'>
            <UploadCloudIcon className='mb-4 h-8 w-8' />
        ...
        {imageStates.map((image, index) => (
          <div
            key={index}
            className='relative aspect-square h-full w-full rounded border-0 bg-slate-200 p-0 shadow-md'
          >
            ...
            {uploading && image.state === 'pending' && (
              <div className='absolute left-0 top-0 flex h-full w-full items-center justify-center rounded-md bg-black bg-opacity-65'>
                <Loader />
              </div>
            )}
